...
from custom_logger.src.data_models.tables import ErrorTable, WarningTable, InfoTable

class DBLogHandler(BaseLogHandler, logging.Handler):
    def __init__(self, db_handler_setup: DBLoggerExtras, structured_data_key="structured_data", *args, **kwargs):
    ...
    def get_insert_data_model(self, log_level_name):
        ...
        log_data_model = self.message_mapping.get(log_level_name.lower(), self.default_message_type)
    ...
    def write_to_db(self, log_level_name, record, db_message):
        ...
        self.db_handler.create_table(table_model=log_data_model, engine=db_engine)
