...
class ExecutionNode:
    def __init__(self, name: str, python_operator: Callable, run_condition: RunCondition=RunCondition.ALL_SUCCESS, configs: dict=None, xargs: list=None):
        self.name = name
    ...
    def select_input_xargs(self, full_xargs):
        return {k:v for k, v in full_xargs.items() if k in self.python_operation.__code__.co_varnames}
    ...
    def run(self, **kwargs):
        ...
        if self.run_condition == RunCondition.ALWAYS:
            ...
            if (self.prev_operations is None) or all([prev_operation.state == RunState.SUCCESS for prev_operation in self.prev_operations]):
                self._run(**kwargs)
            else:
                self.state = RunState.SKIPPED
        else:
            raise NotImplementedError(f"Unknown run condition {self.run_condition}")
        print(f"End {self.name} with status {self.state}...\n----------------------------")
