...
def main():
    with Executor() as executor:
        start_node = ExecutionNode(
            "Initialization step",
            python_operator=add_two_num,
            configs={
                "a": 1,
                "b": 3})
        step_1a = ExecutionNode(
            "Multiply 2a step",
            python_operator=multiply_my_sum,
            xargs=["my_sum"],
            configs={"node_id": "step_2a", "multiplier": 5},
        )
        step_1b = ExecutionNode(
            "Multiply 2b step",
            python_operator=multiply_my_sum,
            xargs=["my_sum"],
            configs={"node_id": "step_2b", "multiplier": 10},
        )
        step_2 = ExecutionNode(
            "Add Result 3 step",
            python_operator=add_up_two_result,
            xargs=["step_2a_result", "step_2b_result"],
        )
